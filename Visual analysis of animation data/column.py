    ...
    for v in cid_dict.values():
        ...
        while True:
            ...
            for item in column_data:
                ...
                if database.execute_sql(table_name=table_name, select="art_id", key="art_id", value=data['art_id']) != 0:
                    print("art_id:{} 重复，跳过".format(data['art_id']))
                    print("-" * 60)
                    # pn += 1
                    continue
                if database.execute_sql(table_name=table_name, mode="insert", keys=list(
                    ...
                    print("art_id:{} 异常，跳过".format(data['art_id']))
                    print("-" * 60)
                    # pn += 1
                    continue
            pn += 1
