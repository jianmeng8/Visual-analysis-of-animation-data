...
class Database(object):  #设置数据库所需连接属性（用户名、端口、密码、数据库等）以及游标
    def __init__(self):
    ...
    def execute_sql(self, table_name, mode="search", select="*", **kwargs):  #操作数据库，负责写入读取数据库
        """
    ...
    def get_cursor(self):  #获取游标
        """
        :return: Cursor Object
        """
        return self.__cursor

    def get_connection(self):  #进行数据库连接
        """
        :return: Connection Object
        """
        return self.__connection

    def close(self):  #断开游标和数据库
        """
